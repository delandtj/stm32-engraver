...
PCB: placed and routed except for 3 pad pairs, as of 2026-09-19
(commit 7f4bf88; the board file is the routing of db1ba76). `graver-controller.kicad_pcb` is GENERATED by the
pipeline in `Hardware/graver-controller/tools/pcb/` (README there is the
manual):
...
HighCurrent >= 0.5 mm, Power >= 0.3 mm), 0 schematic parity, 3
unconnected pad pairs (VDDA pin 9 -> C306 2 mm, PB2 via -> R303 6 mm,
+5V C201 <-> D105 40 mm), 233 vias, every GND pad on the main pour.
VDDA and PB2 are already drawn by copper.py (layered paths) and land in
the board at the next --route run; +5V has no corridor that avoids the
USB pair bands and needs a hand route or a placement change. Fastest
way to finish: draw the 3 by hand in pcbnew, `place.py --export-manual`.
Before the next --route run fix autoroute.py's mop-up, which re-lays
0.3 mm nets at 0.2 mm so the width gate drops them (README, ~5 lines).
The router's result varies 3-10 open between identical runs; eight
passes were spent, do not expect the router alone to reach 0.
...
1. PCB layout and the JLC order: ADR 0003 is the spec (board size, block
   ...
   design"). Next: Jan reviews the board in pcbnew, closes the last 3
   pad pairs by hand (--export-manual), decides on the empty right third
   ...
   export set does not depend on the 3 open pairs and can be scripted
   next (kicad-cli pcb export gerbers / drill / pos, BOM from the
