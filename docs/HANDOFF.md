...
PCB: placed and routed except for 5 pad pairs, as of 2026-09-18
(commit ed2437c). `graver-controller.kicad_pcb` is GENERATED by the
...
State: 0 DRC errors, 0 schematic parity, 5 unconnected pad pairs (GND x2
pour islands at C309/C205, ENC_B x2, +5V), 219 vias. Fastest way to
finish: draw the 5 by hand in pcbnew, then `place.py --export-manual`.
Ugly but working: +3V3 300 mm, ENC_B and NRST long with many vias, the
router's B.Cu tracks dice the pour between MCU and driver, the right
third of the board is empty. Design rules and net classes live in the
.kicad_pro (SaveBoard rewrites it, the scripts restore it).
...
1. PCB layout and the JLC order: ADR 0003 is the spec (board size, block
   ...
   design"). Next: Jan reviews the board in pcbnew, closes the last 5
   ...
   0003 step 8 (Gerber, BOM, CPL, STEP, assembly PDF, test sheet).
