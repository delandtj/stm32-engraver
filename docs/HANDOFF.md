...
PCB: placement only, no copper yet. `graver-controller.kicad_pcb` is
GENERATED by `tools/pcb/place.py` from the netlist (117 parts, footprints
linked to their symbols by UUID, 0 schematic-parity issues, 0 DRC errors,
silkscreen warnings only). Placement is data in that script: nudge the
tables and re-run, do not move parts in pcbnew and expect them to stay.
The script checks courtyards, M3 keepouts and the ADR 0003 distance
criteria and prints a PASS/FAIL table; `tools/pcb/render.sh` renders into
`output/pcb/`. Design rules and net classes (Default, Power, HighCurrent)
are in the `.kicad_pro`; pcbnew's SaveBoard rewrites that file, place.py
restores it. `tools/pcb/routability.sh` autoroutes a scratch COPY with the
router's rule relaxation switched off and reports open nets, vias and
kicad-cli errors: a placement test, not the routing step (it refuses to
run inside the repo). State after pass 3: MCU at (48, 40) rot 90 with
escape corridors, VIN corridor along the rear half, 11 nets still open in
the strict test, all at the QFN and blocked by earlier routes, not by
parts: the 0.5 mm pitch escape needs a scripted radial fanout. Known weak
spots (tools/pcb/README.md): VDDA filter and NRST cap 7-10 mm from their
pins (pushed out for the corridors; the VDDA 100 nF should come back into
the first ring), clamp loop about 26 mm of perimeter (parts sit in a
10 x 12 mm block under J201), Q203 + R205-R208 alone at the right.
J201 is rotated 180: left to right pin 4 GND, 3 NTC, 2 COIL_NEG, 1 VIN.
...
1. PCB layout and the JLC order: ADR 0003 is the spec (board size, block
   ...
   (tools/pcb/place.py). Next: Jan reviews the placement in pcbnew; then
   the copper script (crystal + keepout, 3V3 / 5 V distribution, flyback
   loop, shunt Kelvin, buck, USB pair, U101 EP vias, all locked), then
   KiCadRoutingTools on a COPY for the remaining nets (ADR 0003 Decision
   8 and its risk entry: it rewrites .kicad_pro and grades itself against
   the relaxed rules), grading with kicad-cli only. Still pending on
   loose parts: project footprints for the encoder (12.0 mm lugs), the
   terminal (1.6 mm drills) and the Neutrik (SN pin, thread length vs the
   3 mm wall), USB-C mouth direction against the real part.
